...
//		int floor = sc.nextInt();
		...
		if(select == 1) {
				...
				for(int i = 0; i < 메인객체.주차타워.length; i++) {
					if(메인객체.주차타워[i][0] != null) {System.out.print(메인객체.주차타워[i][0]);}
			...
			int 정산금 = 메인객체.주차계산(금액);
			System.out.println(정산금);
			메인객체.정산금누적(정산금);
			...
			if(isInputRight==true) {	
				ArrayList<매출> 검색결과출력용리스트 =  new ArrayList<>(메인객체.매출출력(year, month, sortBy));
